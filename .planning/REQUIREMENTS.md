...
- [x] **CLOUD-03** _[v1.1]_ — Object-store-backed snapshot storage replacing local-fs snapshots in cloud mode. `ObjectStore::put_stream/get_stream` extensions preserve blake3 content-addressing via incremental hasher. Re-witnessed by `bit_identical_resume_at_step_5_via_{s3,gcs}`.
- [x] **CLOUD-04** _[v1.1]_ — `rollout cloud doctor` CLI subcommand running reachability + auth + write-test against a live cloud.

### Distribution (`DIST-*`)

- [x] **DIST-01** _[v1.1]_ — Coordinator process with persistent state in `Storage` (new namespaces `"work"`, `"epoch"`, `"queue_items"`); one coordinator per run; lease-based exclusion via Postgres single-row lease.
- [x] **DIST-02** _[v1.1]_ — Work-stealing pull queue where idle workers can steal bounded batches from busy peers via the coordinator. CAS-on-state for dedup. CI test: `concurrent_ack_and_steal_no_double_execute`.
- [x] **DIST-03** _[v1.1]_ — Coordinator-restart-from-storage proven by a kill-and-restart integration test on a 3+-node run. Bespoke storage-backed stateless-replayer pattern (no Raft/etcd dependency). Needs architecture spike before phase planning.
- [x] **DIST-04** _[v1.1]_ — Spot-preemption signal handler (AWS IMDSv2 / GCP MDS) that triggers graceful drain: stop-pull → finish-or-requeue → opportunistic snapshot → ack-exit. Two distinct numbers (D-SPOT-01/04): **preemption-notice lead time** = 120s AWS / 30s GCP (the cloud signal returned by `ComputeHint::preemption_signal`); **conservative drain deadline** (what graceful drain completes within) = 60s AWS / 15s GCP — this is the `spot_drain_completes_within_lead_time` test bound, leaving 60s/15s of margin before forced reclaim. v1.1 falls back to TrainState only; process-snapshot path lands with SNAPSHOT-01 in v1.2+.
- [x] **DIST-05** _[v1.1]_ — Split-brain prevention: `worker_self_fence_timeout < coordinator_failure_timeout`; workers validate `coord_epoch` on every RPC; old coord self-aborts on stale-lease detection. Verified by `split_brain_old_coord_self_fences` integration test.

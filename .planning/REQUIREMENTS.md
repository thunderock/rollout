...
- [x] **DIST-04** _[v1.1]_ — Spot-preemption signal handler (AWS IMDSv2 / GCP MDS) that triggers graceful drain: stop-pull → finish-or-requeue → opportunistic snapshot → ack-exit. Budgets: 120s AWS / 30s GCP. v1.1 falls back to TrainState only; process-snapshot path lands with SNAPSHOT-01 in v1.2+.

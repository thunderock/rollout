...
**Success Criteria** (what must be TRUE):
  ...
  3. Operator triggers a mock spot-preemption signal on a worker. Two distinct numbers apply (notice-vs-deadline): the cloud **preemption-notice lead** is 120s AWS / 30s GCP, while the **conservative drain deadline** the worker completes within is 60s AWS / 15s GCP (the `spot_drain_completes_within_lead_time` test bound). The worker stops pulling, requeues in-flight items via lease nack, opportunistically snapshots TrainState if budget allows, deregisters cleanly — surviving workers pick up the requeued items and the run completes (`spot_drain_completes_within_lead_time`).
...
- [x] 07-01-PLAN.md — HARNESS-01: rollout-harness-text env (EchoEnv, MockRewardEnv, env_deterministic_replay)
- [x] 07-02-PLAN.md — HARNESS-02 (sandbox core): layered launcher + curated seccomp allowlist + exec/file tools + macOS stub + CVE negative matrix
- [x] 07-03-PLAN.md — HARNESS-03: rollout-harness-eval (MMLU/IFEval/GSM8K scorers + offline fixtures + eval-as-job + lm-eval parity)
- [x] 07-04-PLAN.md — HARNESS-02 (http): SSRF-filtered hyper connector + http_get/http_post + sandbox-depth matrix README
- [x] 07-05-PLAN.md — Closeout: rollout eval CLI + spec-08 reconcile + mdBook Harnesses chapter + SC4 workspace/14-invariant gate
**UI hint**: no
